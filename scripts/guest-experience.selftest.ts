/**
 * Phase 06 — Guest experience smoke checks (static + optional live client).
 * Run: cd client && npx ts-node --transpile-only scripts/guest-experience.selftest.ts
 * Or: node --import tsx scripts/guest-experience.selftest.ts
 */
import fs from "fs";
...
check("contextual save prompt", guestApp.includes("Sign in to save this problem"));
check("contextual progress prompt", guestApp.includes("track your progress"));
...
check("submit requires auth", workspace.includes("Sign in to submit solutions"));
