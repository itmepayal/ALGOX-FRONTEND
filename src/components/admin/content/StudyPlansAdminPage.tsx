...
export const StudyPlansAdmin: FC = () => {
  ...
  const validateForm = () => {
    ...
    if (!cleanTitle || cleanTitle.length < 2) {
      ...
      next.title = "Enter a real study plan title (not a button label).";
    } else {
      // Reject accidental phrase spam like "Create articleCreate article…"
      for (let len = 1; len <= Math.floor(cleanTitle.length / 2); len += 1) {
        ...
        if (reps >= 2 && unit.repeat(reps) === cleanTitle) {
          next.title =
            "Title looks duplicated. Enter the plan name once.";
          break;
        }
      }
    }
